...
FHIR.oauth2.ready().then(function(client) {
  ...
  client.request("Observation?" + query, {
    ...
    function(ob) {
      ...
      var wgt = retriveValue(byCodes('29463-7'));
      var hgt = retriveValue(byCodes('8302-2'));
      bodyWeight = byCodes('29463-7');
      // var wght= 
      ...
      p.note = retriveNote(byCodes('29463-7'));
    ...
    client.request("/MedicationRequest?patient=" + client.patient.id).then(
      response =>{
        if(response!=null && response.entry != null){
            ...
            response.forEach(c=>{
              medResults.push(c.resource.medicationCodeableConcept.text);
            })
  ...
  function updateObserve(annotation) {
    var date = new Date().toISOString();
    var note = { "time": date, "text": annotation, "authorReference" : null, "authorString": "TestAuthor123"};
    
    if( bodyWeight[0]['note'] != null && bodyWeight[0]['note'] !=undefined){
      bodyWeight[0]['note'].push(note);
    }else{
      bodyWeight[0]['note'] = note;
    }
    var id = bodyWeight[0].id;
    var data = JSON.stringify(bodyWeight[0]);
    var updateQuery = new URLSearchParams();
    updateQuery.set("patient", client.patient.id);
  ...
  function addWeightAnnotation() {
    ...
    updateObserve(annotation);

  }

  function retriveValue(bundle){
    if(bundle != null){
      var value = "";
      var object = bundle[0];
      return value.concat(object.valueQuantity['value']," ",object.valueQuantity['code']);
    }
  }

  function retriveNote(bundle){
    if(bundle != null){
      if( bodyWeight[0]['note'] != null && bodyWeight[0]['note'] !=undefined){
        return bodyWeight[0]['note'].text;
      }
    }
  }
